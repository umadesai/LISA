...
def get_tagged_graph_from_geographical_data(geographical_data):
    """
    Generates a tagged graph of nodes and edges from existing geographical data.

    This calls one of several functions that extract nodes, edges and node/edge attributes from geographical data that exists in different formats (OSM, linear-referenced etc.)  
    """
...
def generate_similar_ideal_data(biker_data, graph):
    """
    Given biker_data, generates ideal_data that has similar initial and end conditions for comparison.

    This calls one of several functions that look at graph properties and generate "ideal cyclist behavior". Ideal cyclist behavior can then be compared to real cyclist behavior.
...
def get_difference_between(biker_data, ideal_data):
    """
    Given biker_data and ideal_data, calculates some numerical value that represents how different they are.

    This calls one of several functions that compare two sets of behavior (e.g. two routes, or two graphs labelled with average daily traffic).
    """
...
def validate(set_of_biker_data, geographical_data, generate_similar_ideal_data):
    ...
    graph is a networkX graph tagged with some attributes.
        attributes can be general like "stress" or "goodness" or specific like "bike_lane_width".
    """
    difference = None
    graph = get_tagged_graph_from_geographical_data(geographical_data)
    set_of_processed_biker_data = translate_biker_data_into_graph_format(set_of_biker_data)
